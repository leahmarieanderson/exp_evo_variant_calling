...
pdf(paste(args[1],'/',args[2],'/',args[2],'_',args[4],'_CopyNumber_', args[3],'bp.pdf',sep=""),height=5, width=18)

hetData <- read.table(paste(args[1],'/',args[2],'/CNV_new_',args[3],'bp/',args[2],'_',args[3],'bp_norm.wig',sep=""), sep='\t', head=FALSE)
hetData1 <- read.table(paste(args[1],'/',args[4],'/CNV_new_',args[3],'bp/',args[4],'_',args[3],'bp_norm.wig',sep=""), sep='\t', head=FALSE)
...
hetPlot <- ggplot() + 
  geom_jitter(data=hetData,
              aes(x=V2,
                  ...
                  alpha = 0.5) +
  geom_jitter(data=hetData1,
              aes(x=V2,
                  y=V3,
                  shape='1',
                  size=1,                  
                  color = ifelse(V1 %% 2 == 0, "C", "D"),
                  fill = ifelse(V1 %% 2 == 0, "C", "D")),
              alpha = 0.5) +
